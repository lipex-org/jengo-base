...
class PestInstaller extends AbstractInstaller
{
    ...
    public function install(): void
    {
        ...
        $modulesConfigPath = APPPATH . 'Config/Modules.php';
        // use a regex to find the composerPackages property and add pestphp/pest to the exclude list
        $isExceptionCaught = rescue(fn() =>
            ClassModifier::fromFile($modulesConfigPath)
                ->mutateArrayProperty(
                    'composerPackages',
                    function (array $packages) {
                        $packages['exclude'] = array_unique(array_merge(
                            $packages['exclude'] ?? [],
                            ['pestphp/pest', 'pestphp/pest-plugin']
                        ));
                        return $packages;
                    }
                )->saveTo($modulesConfigPath), true);
